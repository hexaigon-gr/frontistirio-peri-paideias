...
| `ExpandMap` (`components/expand-map.tsx`) | **Decoration only.** It draws an invented street grid with a pin in the middle and shows nothing about the real location. Fine as an ornament, never as "here is where we are". |
