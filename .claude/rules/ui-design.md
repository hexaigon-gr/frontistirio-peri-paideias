...
| Component | Use for |
| --- | --- |
| `ChalkBulb`, `ChalkAtom`, `ChalkFlask`, `ChalkPlane`, `ChalkPi`, ... (`components/chalk/chalk-marks.tsx`) | Any prominent icon display: section doodles, the landing-page notes, page intros. Raw Lucide icons are only for small inline UI (button icons, form labels, nav items). The old `CircleIcon` is gone, because a circled Lucide glyph is exactly the generic card look this brand avoids. |
...
| ~~`ExpandMap`~~ | **Deleted.** It drew an invented street grid that showed nothing about the real location. Do not bring it back; use `LocationMap`. |
| `LocationMap` (`components/sections/location-map.tsx`) | The real map. Static OpenStreetMap tiles for the actual coordinates, generated by `scripts/generate-map.mjs`, with the pin on the exact point and links out to Google Maps for directions. Regenerate the image whenever the coordinates change. Never embed a raw Google Maps iframe. |
