...
- **Never use a PowerShell here-string (`@'...'@`) in the Bash tool.** Both shells are available in this repo and the syntax is not interchangeable. Bash does not parse `@'`, so it passes a literal `@` as the first argument: the commit lands with `@` as its subject line and a trailing `@`, and `git commit` reports success, so nothing warns you. Multi-line commit messages in Bash go through a heredoc (`git commit -F - <<'EOF'`). Recovering means `git commit --amend` plus `git push --force-with-lease`, which is history rewriting for a typo.
